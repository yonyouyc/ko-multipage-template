...
let _searchlist = {
  person: {
    url: '/cpu-basedocrefer/search/person/getperson',
    type: 'post'
  },
  org: {
    url: '/cpu-basedocrefer/search/org/getorg',
    type: 'post'
  },
  supplydoc: {
    url: '/yuncai/search/supplyDoc/searchByKeyWord?page=0&pagesize=10',
    type: 'get'
  }
}

/* global ko $ */
function init ({type, searchlist, delay = 300, showtext, value, placeholder = '请输入要查询的内容', showrefer = function () { }, onselect = function () {}}) {
  let defaultValue = ''
  // 初始化
  if (searchlist) {
  ...
  // 输入改变事件
  this.queryvalue.subscribe(_.debounce(function (val) {
    if (val && defaultValue) {
      ...
      Get(_searchlist[type].url, {keyword: key()}).then(
        function (response) {
          if (response.success === true) {
            let data = _.map(response.result.data, function (item) {
              return {
                id: item.id,
                name: item.name
              }
      ...
      $.ajax({
        ...
        success (response) {
          if (response.status === 1) {
            let data = _.map(response.data, function (item) {
              return {
                id: item.id,
                name: item.name
              }
